...
export default function PasseioAdd() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const resAtracoes = await fetch(`${API_URL}/atracoes/panel?limit=100&all=true`, {
          headers: {
        ...
        const resGuias = await fetch(`${API_URL}/guide/panel?limit=100`);
        if (resGuias.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Erro ao criar passeio");

      const passeioId = data?.passeio?.id;
      if (passeioId && imagens.length > 0) {
        const form = new FormData();
        imagens.forEach((img) => {
          const arquivo = img.file || img;
          form.append("image", arquivo);
        });
        await fetch(`${API_URL}/passeios/${passeioId}/imagens`, {
          method: "POST",
          body: form,
        });
      }

      Swal.fire("Sucesso", "Passeio criado com sucesso!", "success");
      navigate("/guia/panel/passeios");
    } catch (err) {
      console.error(err);
      Swal.fire("Erro", err.message || "Erro ao criar passeio", "error");
    } finally {
        ...
        <div className="w-full max-w-[900px] mx-auto my-12 p-6 md:p-10 bg-white rounded-2xl shadow-xl shadow-black/5 font-poppins">
          <header className="mb-10">
            ...
            <p className="text-gray-500 mt-2">
              Preencha as informações do novo passeio para publicação.
            </p>
          </header>
